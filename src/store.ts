// TweakPine Store - Event-based state management (no React)
...
export type TweakValue = number | boolean | string | SpringConfig | ActionConfig | SelectConfig | ColorConfig | TextConfig;

export type TweakConfig = {
  [key: string]: TweakValue | [number, number, number, number?] | TweakConfig;
};
...
type Listener = () => void;
type ChangeListener = (values: Record<string, TweakValue>) => void;
...
class TweakPineStoreClass {
  ...
  registerPanel(id: string, name: string, config: TweakConfig): void {
  ...
  updateValue(panelId: string, path: string, value: TweakValue): void {
    ...
    this.notify(panelId);
    this.notifyChange(panelId, panel.values);
  }
  ...
  subscribeChange(panelId: string, listener: ChangeListener): () => void {
    if (!this.changeListeners.has(panelId)) {
      this.changeListeners.set(panelId, new Set());
    }
    this.changeListeners.get(panelId)!.add(listener);

    return () => {
      this.changeListeners.get(panelId)?.delete(listener);
    };
  }
  ...
  private notifyChange(panelId: string, values: Record<string, TweakValue>): void {
    this.changeListeners.get(panelId)?.forEach(fn => fn(values));
  }
  ...
  private parseConfig(config: TweakConfig, prefix: string): ControlMeta[] {
...
// Singleton instance
export const TweakPineStore = new TweakPineStoreClass();
